...
def mst(maze, currX, currY, fruit_list):
    list = []
    list.append((currX, currY))
    for fruit in fruit_list:
        list.append(fruit)
    distance = {}
    for i in range(0, len(list)):
        for j in range(i + 1, len(list)):
            d = Astar(maze, list[i][0], list[i][1], list[j][0], list[j][1])
            distance[(list[i], list[j])] = d
            distance[(list[j], list[i])] = d
...
if __name__ == "__main__":
    ...
    t_value[(startX, startY, tuple(points_position.items()))] = mst(maze, startX, startY, orig_fruit_list)
    ...
    while len(list) != 0:
        ...
        for fruit in fruit_list1:
            if fruit[0] == currX and fruit[1] == currY:
                ...
                t_value[(currX, currY, tuple(new_dict.items()))] = f_value[(currX, currY, tuple(new_dict.items()))] + mst(maze, currX, currY, new_fruit_list)

                parent[(currX, currY, tuple(new_dict.items()))] = parent[(currX, currY, tuple(curr[2].items()))]
                curr = (currX, currY, collections.OrderedDict(new_dict))
        ...
        if currX - 1 >= 0 and maze[currY][currX - 1] != '%':
            ...
            if visited[(currX - 1, currY, orig)] != 1:
                ...
                if (currX - 1, currY, orig) not in f_value:
                    ...
                    t_value[(currX - 1, currY, orig)] = f_value[(currX - 1, currY, orig)] + mst(maze, currX - 1, currY, fruit_list)
        ...
        if currY - 1 >= 0 and maze[currY - 1][currX] != '%':
            ...
            if visited[(currX, currY - 1, orig)] != 1:
                ...
                if (currX, currY - 1, orig) not in f_value:
                    ...
                    t_value[(currX, currY - 1, orig)] = f_value[(currX, currY - 1, orig)] + mst(maze, currX, currY - 1, fruit_list)
